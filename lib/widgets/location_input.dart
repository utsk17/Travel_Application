...
class _LocationInputState extends State<LocationInput> {
  ...
  String get locationImage {
    ...
    //ensure below return is in single line
  ...
  Future<void> _savePlace(double latitude, double longitude) async {
    final url = Uri.parse(
        'https://maps.googleapis.com/maps/api/geocode/json?latlng=$latitude,$longitude&key=AIzaSyCDQwXURcHYwFrC9E_z09h2niAaVA3WesU');

    final response = await http.get(url);
    final resData = json.decode(response.body);
    final address = resData['results'][0]
        ['formatted_address']; //this is the human readable address
  ...
  //locationData here is an object with many properties
  //such as the latitude and longitude of the location
  ...
  Widget build(BuildContext context) {
    ...
    //given below we use preview content widget
    if (_pickedLocation != null) {
